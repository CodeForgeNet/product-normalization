...
if __name__ == "__main__":
    print("="*70)
    print("TEXT NORMALIZATION MODULE - DEMO")
    print("="*70)
    print()
    ...
    test_cases = [
        {
            ...
            'product': 'Face Wash Purifying Neem Pack 100ml',
            'quantity': '100 ml'
        },
        {
            'brand': 'Tata Tea',
            'product': 'Tata Tea Gold Premium Pack 500g',
            'quantity': '500g'
        },
        {
            'brand': 'Amul',
            'product': 'Amul Butter Pack 100g',
            'quantity': '100 g'
        },
        {
            'brand': 'Mother Dairy',
            'product': 'Mother Dairy Fresh Milk Full Cream',
            'quantity': '500 ml'
        },
        {
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        brand_norm = normalizer.normalize_brand(test['brand'])
        product_norm = normalizer.normalize_product_name(test['product'])
        qty_norm = normalizer.normalize_quantity(test['quantity'])
        fingerprint = normalizer.create_fingerprint(test['brand'], test['product'], test['quantity'])
        
        print(f"  Normalized:")
        print(f"    Brand: {brand_norm}")
        print(f"    Product: {product_norm}")
    ...
    for qty1, qty2, expected in qty_tests:
        result = normalizer.quantities_match(qty1, qty2)
        status = "✅" if result == expected else "❌"
        print(f"{status} '{qty1}' == '{qty2}': {result} (expected: {expected})")
